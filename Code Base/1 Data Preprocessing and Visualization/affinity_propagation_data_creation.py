...
def create_coase_rf():
    ...
    mess_points = []
    orientations = []
    select_stmt = "SELECT reference_points, messurement_points, orientation FROM indoor_nav_using_affinity_prop.evaluation_master ORDER BY reference_points ASC;"
    ...
    #print(ori_rp)
    select_cluster = "SELECT * FROM indoor_nav_using_affinity_prop.evaluation_master where reference_points = %(reference_points)s and messurement_points = %(messurement_points)s and orientation = %(orientation)s"
    ...
    for rf in refference_labels:
        for mp in messssurement_points:
            for orp in ori_rp:
                ...
                test_mean.to_sql('affinity_evaluation_master',con=engine,index=False,if_exists='append')
                #print(test_mean)
    my_cusor.close()   
